 ...
 <div class="container">
   <ul  class="filmList">
     <li v-for="(film,index) in filmList" :key="index">
       ...
       <div class="center">
         <p class="title">{{film.name}} <span class="bbb">{{film.filmType.name}}</span></p>
         <p class="score"><span v-if="film.grade">观众评分：</span><span class="sss">{{film.grade}}</span></p>
         <p class="actor">主演：
...
export default {
  ...
  mounted() {
    //发送axios请求，数据
    this.axios({
      url:'https://m.maizuo.com/gateway?cityId=310100&pageNum=1&pageSize=50&type=1&k=2283804',
      method:'GET',
      headers:{
